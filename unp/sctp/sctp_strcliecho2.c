...
* All right reserved.
* 
* File Name: sctp_strcliecho2.c
* Version: 
* Writen by Canux canuxcheng@gmail.com
* Created Time: Thu 25 Sep 2014 10:10:33 PM CST
* Description: 
************************************************************************/
#include "all.h"
...
void sctpstr_cli_echoall(FILE *fp, int sock_fd, struct sockaddr *to, socklen_t tolen)
{
	...
	while (fgets(sendline, SCTP_MAXLINE - 9, fp) != NULL)
	{
		...
		for (i = 0; i < SERV_MAX_SCTP_STRM * 2; i++)
		{
			...
			printf("From stream:%d seq:%d (assoc:0x%x):", sri.sinfo_stream, sri.sinfo_ssn, (u_int)sri.sinfo_assoc_id);
			printf("%.*s\n", rd_sz, recvline);
		}
	}
